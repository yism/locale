...
test("verification rejects invalid signatures and unknown kids deterministically", async () => {
  ...
  const parsedPayload = JSON.parse(decodeBase64Url(payload).toString("utf8"));
  parsedPayload.mid = "mission_tampered";
  const tampered = `${header}.${encodeBase64Url(JSON.stringify(parsedPayload))}.${signature}`;
